...
await sharp(buf).resize(32, 32, { fit: 'cover' }).png().toFile(join(OUT_DIR, 'favicon.png'))
...
// PWA icons
await sharp(buf).resize(192, 192, { fit: 'cover' }).png().toFile(join(OUT_DIR, 'icon-192.png'))

await sharp(buf).resize(512, 512, { fit: 'cover' }).png().toFile(join(OUT_DIR, 'icon-512.png'))

// Also create an SVG favicon that references the PNG
// For simplicity, we'll also create a favicon.ico-sized png
await sharp(buf).resize(48, 48, { fit: 'cover' }).png().toFile(join(OUT_DIR, 'favicon-48.png'))
